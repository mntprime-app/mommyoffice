...
## BUG-091 — Security: cf-key.txt credential dump found untracked (RESOLVED 2026-09-18, Session 35)

**Severity:** Critical (credentials in working tree)

**Discovery:** During session close audit, `git status` revealed `cf-key.txt` as an untracked file in the project root. File contained the Cloudflare Stream signing key pair (key ID + PEM private key) that was generated by `generate-cf-key.ps1` during BUG-089 investigation.

**Risk:** If committed, CF Stream signing credentials would be permanently in git history and visible to anyone with repo access.

**Resolution:**
1. Added `.gitignore` entries: `cf-key.txt`, `*.key.txt`, `*-key.txt`, `*-secret.txt`, `*-credentials.txt`
2. Deleted `cf-key.txt` from disk
3. Committed `.gitignore` update: commit `54b2586`

**Impact:** None — CF JWT signing was already removed in BUG-089 final resolution (Session 25). The keys in `cf-key.txt` were never deployed to Vercel env vars or used in production. CF Stream runs in unsigned direct-embed mode.

**Permanent rule added to .gitignore:**
```
# Credential dump files — NEVER commit
cf-key.txt
*.key.txt
*-key.txt
*-secret.txt
