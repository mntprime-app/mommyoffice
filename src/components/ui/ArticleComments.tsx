...
import { getArticleComments, addArticleComment, deleteArticleComment } from '@/app/actions/articles';
import type { ArticleComment } from '@/app/actions/articles';
import { createClient } from '@/lib/supabase/client';

const s = {
  section: { marginTop: '2.5rem', borderTop: '1px solid #2a2a2a', paddingTop: '1.75rem' } as React.CSSProperties,
  heading: { fontSize: '16px', fontWeight: 700, color: '#e5e5e5', marginBottom: '1.25rem' } as React.CSSProperties,
...
export default function ArticleComments({
  ...
  useEffect(() => {
    ...
    if (commentsEnabled) {
      getArticleComments(articleId).then((data) => { setComments(data); setLoading(false); });
    } else {
      setLoading(false);
    }
  ...
  if (!commentsEnabled) {
    return (
      <div style={s.section}>
        <div style={s.disabled}>💬 Энэ нийтлэлд сэтгэгдэл бичих боломжгүй байна.</div>
      </div>
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    setSubmitError('');
    const { comment, error } = await addArticleComment(articleId, body.trim());
  ...
  async function handleDelete(commentId: string) {
    ...
    const { error } = await deleteArticleComment(commentId);
