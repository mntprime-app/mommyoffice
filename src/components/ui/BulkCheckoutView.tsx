...
export function BulkCheckoutView({ locale, courses }: BulkCheckoutViewProps) {
  ...
  if (step === 'qr') return (
    <div className="mo-checkout-wrap" style={{ maxWidth: '920px', margin: '0 auto', padding: '3rem 2rem' }}>
      <h1 style={{ fontSize: '26px', fontWeight: 800, color: '#e5e5e5', margin: '0 0 0.5rem' }}>📱 QPay QR кодоор төлнө үү</h1>
      <p style={{ fontSize: '14px', color: '#666', margin: '0 0 2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '2.5rem', alignItems: 'flex-start' }}>
        <div>
          <div className="mo-qr-wrapper" style={{ marginBottom: '24px' }}>
            <div style={{ background: '#fff', borderRadius: '20px', padding: '24px', display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '16px', boxShadow: '0 8px 40px rgba(0,181,173,0.2)' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: '#333', margin: '0 0 2px' }}>₮{total.toLocaleString()} төлнө үү</p>
                <p style={{ fontSize: '11px', color: '#999', margin: 0 }}>{courses.length} сургалт · QPay апп → QR скан</p>
              </div>
            </div>
          </div>
          <div style={{ background: '#1a1a1a', border: '1px solid #222', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            ...
            {[['1','QPay аппыг нээнэ үү'],['2','QR код скан хийнэ үү'],['3','Дүнг баталгаажуулна уу'],['4','Төлбөр амжилттай — и-мэйл ирнэ']].map(([n, t]) => (
