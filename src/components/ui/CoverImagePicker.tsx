...
// ── Combined upload + preview section (named export) ─────────────────────────
// Drop this as a full-width section below the admin form grid.
// Replaces both CoverImagePicker (in SideCard) + CoverImagePreview (at bottom).
...
export function CoverImageSection({ value, onChange, title, badge }: SectionProps) {
  const [mode, setMode]             = useState<Mode>('url');
  const [dragging, setDragging]     = useState(false);
  const [uploading, setUploading]   = useState(false);
  const [uploadErr, setUploadErr]   = useState('');
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    setUploading(true); setProgress(15);
    let blob: Blob;
    try { blob = await compressImage(file, 1920, 1080); setCompressed({ before: file.size, after: blob.size }); }
    catch { blob = file; }
    setProgress(40);
    const supabase = createClient();
    const fname = `covers/${Date.now()}-${Math.random().toString(36).slice(2)}.webp`;
    ...
    const { data: urlData } = supabase.storage.from('media').getPublicUrl(data.path);
    onChange(urlData.publicUrl);
    setUploading(false); setMode('url');
    setTimeout(() => setProgress(0), 800);
  }, [onChange]);
  ...
  return (
    <div style={{ marginTop: '2rem', background: '#161616', border: '1px solid #2a2a2a', borderRadius: '16px', padding: '20px 24px 24px' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingBottom: '14px', marginBottom: '16px', borderBottom: '1px solid #2a2a2a' }}>
        <span style={{ fontSize: '13px', fontWeight: 700, color: '#00B5AD' }}>🖼 COVER IMAGE</span>
        <span style={{ fontSize: '11px', color: '#6b7280' }}>— upload &amp; live preview</span>
      </div>

      {/* Two-column: upload controls left, preview right */}
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '24px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {/* Mode tabs */}
          <div style={{ display: 'flex', gap: '2px', background: '#111', padding: '2px', borderRadius: '7px', border: '1px solid #2a2a2a', alignSelf: 'flex-start' }}>
            {(['url', 'upload'] as Mode[]).map(m => (
              <button key={m} type="button" onClick={() => setMode(m)} style={{
                padding: '4px 14px', borderRadius: '5px', fontSize: '11px', fontWeight: 600,
                border: 'none', cursor: 'pointer',
                background: mode === m ? '#2a2a2a' : 'transparent',
                color:       mode === m ? '#e5e5e5' : '#6b7280',
                transition: 'all 0.12s',
          ...
          {mode === 'url' ? (
            <div>
              <input
                type="url" value={value} onChange={e => onChange(e.target.value)}
                placeholder="https://cdn.example.com/cover.webp"
                style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', boxSizing: 'border-box', border: `1px solid ${value ? 'rgba(0,181,173,0.4)' : '#2a2a2a'}`, background: '#111', color: '#e5e5e5', fontSize: '12px', outline: 'none', fontFamily: 'inherit' }}
              />
              {value && (
                <button type="button" onClick={() => onChange('')} style={{ marginTop: '6px', background: 'none', border: 'none', color: '#6b7280', fontSize: '11px', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
                  Устгах
                </button>
              )}
              {compressed && (
                <p style={{ fontSize: '10px', color: '#10b981', margin: '6px 0 0' }}>
                  ✓ {(compressed.before/1024).toFixed(0)}KB → {(compressed.after/1024).toFixed(0)}KB · WebP
                </p>
              )}
            </div>
            ...
            <div
              onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
              onClick={() => !uploading && fileRef.current?.click()}
              style={{ border: `2px dashed ${dragging ? '#00B5AD' : uploading ? 'rgba(0,181,173,0.4)' : '#2a2a2a'}`, borderRadius: '10px', padding: '28px 16px', textAlign: 'center', cursor: uploading ? 'wait' : 'pointer', background: dragging ? 'rgba(0,181,173,0.05)' : '#111', transition: 'all 0.15s' }}
            >
              <input ref={fileRef} type="file" accept={ACCEPTED} style={{ display: 'none' }} onChange={onFileChange} />
              {uploading ? (
                <div>
                  <div style={{ fontSize: '22px', marginBottom: '8px' }}>⏳</div>
                  <p style={{ fontSize: '12px', color: '#9ca3af', margin: '0 0 10px' }}>Байршуулж байна...</p>
                  <div style={{ height: '3px', background: '#2a2a2a', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${progress}%`, background: '#00B5AD', borderRadius: '2px', transition: 'width 0.3s' }} />
                  </div>
                </div>
                ...
                <>
                  ...
                  <p style={{ fontSize: '12px', fontWeight: 600, color: '#e5e5e5', margin: '0 0 4px' }}>
                    {dragging ? 'Зургаа оруулна уу' : 'Зургаа энд чирэх'}
                  </p>
              ...
              {uploadErr && (
                <p style={{ fontSize: '11px', color: '#fca5a5', marginTop: '10px', background: 'rgba(239,68,68,0.1)', padding: '6px 10px', borderRadius: '6px' }}>
                  {uploadErr}
                </p>
              )}
            </div>
          ...
          {/* Priority tip */}
          <div style={{ fontSize: '10px', color: '#6b7280', padding: '7px 10px', background: 'rgba(0,181,173,0.04)', borderRadius: '6px', border: '1px solid rgba(0,181,173,0.08)', lineHeight: 1.5 }}>
            💡 <strong style={{ color: '#9ca3af' }}>Thumbnail Priority:</strong> Custom upload = 100%. YouTube auto-thumbnail = fallback only if empty.
          </div>
        </div>

        {/* ── Right: live preview ── */}
        <div>
          <div style={{ fontSize: '10px', fontWeight: 700, color: '#6b7280', letterSpacing: '0.5px', marginBottom: '8px' }}>👁 LIVE PREVIEW — 1:1 production layout</div>
          <div style={{ position: 'relative', width: '100%', paddingBottom: '56.25%', borderRadius: '12px', overflow: 'hidden', background: '#0d0d0d', border: '1px solid #2a2a2a', boxShadow: '0 4px 24px rgba(0,0,0,0.5)' }}>
            {value ? (
              <>
                <img src={value} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', display: 'block' }} onError={e => { (e.target as HTMLImageElement).style.opacity = '0'; }} />
                ...
                <div style={{ position: 'absolute', bottom: '18px', left: '18px', maxWidth: '55%' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                <div style={{ fontSize: '40px', opacity: 0.07 }}>🖼️</div>
                <p style={{ fontSize: '12px', color: '#374151', margin: 0, textAlign: 'center' }}>Preview энд харагдана<br /><span style={{ fontSize: '10px', color: '#1f2937' }}>Зургаа оруулсны дараа шууд харагдана</span></p>
              </div>
            )}
          </div>
        </div>
...
// ── Legacy full-width preview (kept for backward compat) ──────────────────────
