...
import { reactToArticle, getMyArticleReaction } from '@/app/actions/articles';
...
export default function ArticleReactions({
  articleId, initialSuper, initialUp, initialDown, locale,
}: Props) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const existing = await getMyArticleReaction(articleId);
      setReacted(existing);
    });
  }, [articleId]);
  ...
  async function handleReact(type: ReactionType) {
    ...
    setClicking(true);
    const { counts: updated, alreadyReacted } = await reactToArticle(articleId, type);
    if (alreadyReacted) {
      setReacted(type); // DB says already voted — sync UI
    } else if (updated) {
      setCounts(updated);
      setReacted(type);
    }
