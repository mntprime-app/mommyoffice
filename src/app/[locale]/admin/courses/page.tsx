      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '0.25rem' }}>
            <Link href={`/${locale}/admin`} style={{ color: 'var(--teal)', textDecoration: 'none' }}>Admin</Link> / Хичээлүүд
          </div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800 }}>Хичээлүүд ({courses?.length || 0})</h1>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {(courses || []).map((course, i) => (
              <tr key={course.id} style={{ borderBottom: i < (courses?.length || 1) - 1 ? '1px solid var(--border)' : 'none' }}>
                <td style={{ padding: '14px 16px', fontWeight: 600, fontSize: '14px' }}>{course.title_mn}</td>
                <td style={{ padding: '14px 16px', fontSize: '13px', color: '#6b7280' }}>
                  <span style={{ background: 'var(--teal-light)', color: 'var(--teal)', padding: '2px 10px', borderRadius: '10px', fontWeight: 600 }}>
                    {course.category}
                  </span>
                </td>
                ...
                <td style={{ padding: '14px 16px' }}>
                  <Link href={`/${locale}/admin/courses/${course.id}/edit`} style={{
                    color: 'var(--teal)', textDecoration: 'none', fontSize: '13px', fontWeight: 600
                  }}>
                    Засах
                  </Link>
                </td>
