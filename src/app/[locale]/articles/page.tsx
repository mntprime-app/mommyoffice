...
function ArticleScrollCard({ a, locale }: { a: Record<string, unknown>; locale: string }) {
  ...
  return (
    <Link href={href} style={{ textDecoration: 'none', flexShrink: 0, width: '210px', display: 'flex' }}>
      <article className="netflix-card" style={{
        borderRadius: '8px', overflow: 'hidden',
        background: '#1a1a1a', border: '1px solid #2a2a2a', width: '100%',
      }}>
        <div style={{
          height: '125px', background: grad,
          ...
          <span style={{
            ...
            color: '#00B5AD', fontSize: '9px', fontWeight: 700,
            padding: '2px 7px', borderRadius: '3px', textTransform: 'uppercase', letterSpacing: '0.5px',
          }}>{cat}</span>
        </div>
...
// ── Section row (Trending / per-category) ──
function ArticleRow({
  title, badge, articles, locale, viewAllHref,
}: {
  title: string; badge?: string; articles: Record<string, unknown>[]; locale: string; viewAllHref: string;
}) {
  if (articles.length === 0) return null;
  return (
    <section style={{ padding: '0 4%', marginBottom: '2.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#e5e5e5', margin: 0 }}>{title}</h2>
          {badge && (
            <span style={{
              fontSize: '10px', fontWeight: 700, padding: '2px 8px',
              background: 'rgba(0,181,173,0.12)', color: '#00B5AD',
              border: '1px solid rgba(0,181,173,0.3)', borderRadius: '4px', letterSpacing: '1px',
            }}>{badge}</span>
          )}
        </div>
        <Link href={viewAllHref} style={{ fontSize: '13px', color: '#00B5AD', fontWeight: 600, textDecoration: 'none' }}>
          Бүгдийг харах ›
        </Link>
      </div>
      <div style={{
        ...
        {articles.map((a, i) => (
          <ArticleScrollCard key={String(a.id || i)} a={a} locale={locale} />
        ))}
      </div>
    </section>
  );
...
// ── Grid card (used in filter mode) ──
function ArticleGridCard({ a, locale, t }: { a: Record<string, unknown>; locale: string; t: (k: string) => string }) {
  const title   = locale === 'mn' ? String(a.title_mn || '') : String(a.title_en || a.title_mn || '');
    ...
    <Link href={href} style={{ textDecoration: 'none', display: 'flex' }}>
      <article className="netflix-card" style={{
        ...
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          position: 'relative', overflow: 'hidden', fontSize: '3.5rem',
        }}>
          {a.cover_image_url
            ? <img src={String(a.cover_image_url)} alt={title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : <span>{String(a.emoji || '✨')}</span>
          ...
          <span style={{
            ...
            padding: '3px 9px', borderRadius: '4px', textTransform: 'uppercase', letterSpacing: '0.5px',
          }}>{cat}</span>
        </div>
        <div style={{ padding: '1rem 1.1rem 1.1rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
          {date && <span style={{ fontSize: '11px', color: '#666', marginBottom: '6px' }}>{date}</span>}
          <h2 style={{
            fontSize: '14px', fontWeight: 700, color: '#e5e5e5',
            lineHeight: 1.5, marginBottom: '6px',
            display: '-webkit-box', WebkitLineClamp: 2,
            WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: '42px',
          }}>{title}</h2>
          {excerpt && (
            <p style={{
          ...
          <span style={{ display: 'inline-block', marginTop: 'auto', paddingTop: '10px', color: '#00B5AD', fontSize: '12px', fontWeight: 700 }}>
            {t('read_more')} →
          </span>
        </div>
      </article>
...
export default async function ArticlesPage({
  ...
  // Group by category for browse mode rows
  const catMap: Record<string, Record<string, unknown>[]> = {};
  CATEGORIES.slice(1).forEach(cat => {
    catMap[cat] = allArticles.filter(a => String(a.category || '') === cat);
  });
  const trending = allArticles.slice(0, 8);

  // Hero: use first article of filtered set, else first overall
  const featured = displayArticles[0] || allArticles[0];
  ...
  return (
    <div style={{ background: '#141414', minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: featuredGrad }}>
          {featured?.cover_image_url ? (
            <img src={String(featured.cover_image_url)} alt={featuredTitle}
              style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.45 }} />
          ) : (
            <div style={{
              position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
              display: 'flex', alignItems: 'center', justifyContent: 'flex-end',
              paddingRight: '8%', opacity: 0.15, fontSize: 'min(40vw, 320px)',
            }}>
        ...
        <div style={{ position: 'absolute', bottom: '22%', left: '4%', maxWidth: '540px', zIndex: 2 }}>
          <span style={{
          ...
          {featuredExcerpt && (
            <p style={{ fontSize: '15px', color: '#b0bcc8', lineHeight: 1.65, marginBottom: '1.5rem', maxWidth: '440px' }}>
              {featuredExcerpt}
            </p>
          )}
          <Link href={String(featuredSlug)} style={{
      ...
      {/* ── BROWSE MODE: editorial rows ── */}
      {!isFiltered && (
        <div style={{ paddingTop: '2rem' }}>
          <ArticleRow
            title="Трендийн нийтлэлүүд"
            badge="TRENDING"
            articles={trending}
            locale={locale}
      ...
      {isFiltered && (
        <div style={{ padding: '2rem 4%' }}>
          {displayArticles.length === 0 ? (
            <p style={{ color: '#666', textAlign: 'center', padding: '3rem 0' }}>Нийтлэл олдсонгүй</p>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '1.5rem' }}>
              {displayArticles.map((a, i) => (
                <ArticleGridCard key={String(a.id || i)} a={a} locale={locale} t={(k) => t(k)} />
              ))}
            </div>
          )}
        </div>
